      ...
      try {
        ...
        const myLidBase =
          ...
          (sock?.authState?.creds?.me?.lid
            ? baseLid(sock.authState.creds.me.lid)
            : null) ||
        ...
        const mentionedByJid = mentionedJids.some((m) => {
          try {
            ...
            if (myJidNormalized && areJidsSameUser(norm, myJidNormalized)) {
              return true;
            }
            return myDigits ? digits(norm) === myDigits : false;
          } catch {
            if (myJidNormalized && areJidsSameUser(m, myJidNormalized)) {
              return true;
            }
        ...
        // fallback final: se houver qualquer mentionedJid/mentionedLid, considera menção
        if (!mentioned) {
          const hasAnyMention =
            (mentionedJids && mentionedJids.length > 0) ||
            (mentionedLids && mentionedLids.length > 0);
          if (hasAnyMention) mentioned = true;
        }

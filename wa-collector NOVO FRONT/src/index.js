...
// Desempacota mensagens encapusladas (ephemeral/viewOnce/editadas)
const unwrapMessage = (m) => {
  let cur = m;
  // desenrola camadas comuns
  ...
  if (cur?.editedMessage?.message) cur = cur.editedMessage.message;
  return cur || m;
};
    ...
    for (const msg of messages) {
      try {
        ...
        const myJidNormalized = myJidForCompare
          ...
          : null;
        const myJidNoDevice =
          myJidNormalized && myJidNormalized.includes(':')
            ? `${myJidNormalized.split(':')[0]}@s.whatsapp.net`
            : myJidNormalized;
        ...
        const mentionedByJid = mentionedJids.some((m) => {
          try {
            ...
            if (myJidNoDevice && areJidsSameUser(norm, myJidNoDevice)) return true;
            return myDigits ? digits(norm) === myDigits : false;
          } catch {
            if (myJidNormalized && areJidsSameUser(m, myJidNormalized)) return true;
            if (myJidNoDevice && areJidsSameUser(m, myJidNoDevice)) return true;
        ...
        // Recalculo mais tolerante (lida com jid com/sem device e LID)
        {
          const selfJids = new Set();
          const selfDigits = new Set();
          const addJid = (jid) => {
            if (!jid) return;
            ...
            if (norm.includes(':') && norm.includes('@')) {
              ...
              selfJids.add(`${base}@${domain}`);
            }
            const d = digits(norm);
            if (d) selfDigits.add(d);
          };
          addJid(MY_JID);
          addJid(sock?.user?.id);
          addJid(sock?.user?.lid);

          const selfLids = new Set();
          const addLid = (lid) => {
            if (!lid) return;
            const b = baseLid(lid);
            if (b) selfLids.add(b);
          };
          addLid(sock?.user?.lid);
          addLid(sock?.user?.id);
          addLid(MY_LID_ENV);

          const mentionedByJidLoose = mentionedJids.some((m) => {
            try {
              const norm = jidNormalizedUser(m);
              return (
                Array.from(selfJids).some((sj) => areJidsSameUser(norm, sj)) ||
                selfDigits.has(digits(norm))
              );
            } catch {
              return (
                Array.from(selfJids).some((sj) => areJidsSameUser(m, sj)) ||
                selfDigits.has(digits(m))
              );
            }
          });

          const mentionedByLidLoose =
            Array.from(selfLids).length > 0 &&
            (mentionedJids.some((m) => selfLids.has(baseLid(m))) ||
              mentionedLids.some((m) => selfLids.has(baseLid(m))));

          const mentionedByTextLoose = Array.from(selfDigits).some((d) =>
            d ? text.replace(/\D/g, '').includes(d) : false,
          );

          if (mentionedByJidLoose || mentionedByLidLoose || mentionedByTextLoose) {
            mentioned = true;
          }
        }

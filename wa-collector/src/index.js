...
// FORWARD_ALL=false -> só manda quando tiver @mention nativa
// FORWARD_ALL=true  -> manda TUDO (debug)
const FORWARD_ALL = parseBoolEnv(process.env.FORWARD_ALL, false);

// Regex opcional para detectar "tags" ou palavras-chave no texto
// Exemplo de TAG_REGEX: "#suporte|@bmp"
const TAG_REGEX_RAW = process.env.TAG_REGEX || '';
let TAG_REGEX = null;
if (TAG_REGEX_RAW) {
  try {
    TAG_REGEX = new RegExp(TAG_REGEX_RAW, 'i');
    logger.info({ TAG_REGEX_RAW }, 'TAG_REGEX carregado com sucesso');
  } catch (err) {
    logger.error({ err, TAG_REGEX_RAW }, 'TAG_REGEX inválido, ignorando');
  }
}

// Número configurável da instância (apenas dígitos), usado para detectar mention por texto
// Ex.: "+55 11 98888-7777" -> "5511988887777"
const MY_PHONE = (process.env.MY_PHONE || '').replace(/\D/g, '') || null;
...
async function createSock() {
  ...
  sock = makeWASocket({
    ...
    printQRInTerminal: false, // QR só via HTTP
  });
  ...
  // Logger cru de mensagens (pra debug)
  sock.ev.on('messages.upsert', (m) => {
    logger.info(
      ...
      'messages.upsert (logger bruto)'
    );
  });

  // Handler filtrado: só grupo + só quando tiver @mention nativa
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    if (type !== 'notify') {
      logger.info({ type }, 'Ignorando messages.upsert que não é notify');
      return;
    }
    ...
    for (const msg of messages) {
      try {
        ...
        if (!jid?.endsWith('@g.us')) {
          logger.info({ jid }, 'Ignorando mensagem que não é de grupo');
          continue;
        }
        ...
        // Dados da própria instância
        const myNumberDigits =
          MY_PHONE || myDigits || normalizeJidToDigits(sock?.user?.id || '');
        const myBareJid =
          myJid || (sock?.user?.id && jidNormalizedUser(sock.user.id));

        // Logar o que veio de mention
        logger.info(
          {
            myJid,
            myBareJid,
        ...
        // Menção nativa via mentionedJid (@ no WhatsApp)
        let mentionedByContext = false;
        if (Array.isArray(mentionedJids) && mentionedJids.length > 0) {
          for (const mj of mentionedJids) {
            const mjNorm = jidNormalizedUser(mj);
            const mjDigits = normalizeJidToDigits(mj);
            logger.info(
              { mj, mjNorm, mjDigits, myBareJid, myNumberDigits },
              'Comparando mentionedJid com meu número'
            );

            const sameJid = myBareJid && mjNorm && mjNorm === myBareJid;
            const sameDigits =
              myNumberDigits && mjDigits && mjDigits === myNumberDigits;

            if (sameJid || sameDigits) {
        ...
        let mentionedByTag = false;
        let mentionedByPhone = false;

        const mentioned =
          mentionedByContext || mentionedByTag || mentionedByPhone;

        // Menção por texto, usando TAG_REGEX (se configurado)
        mentionedByTag = false;
        if (TAG_REGEX && text) {
          mentionedByTag = TAG_REGEX.test(text);
        }

        // Menção por texto contendo o número da instância
        mentionedByPhone = false;
        if (myNumberDigits && text) {
          const textDigits = digits(text);
          if (textDigits && textDigits.includes(myNumberDigits)) {
            mentionedByPhone = true;
          }
        }
        ...
        // - senão -> só se mentionedByContext=true (menção nativa @)
        const shouldForward =
          FORWARD_ALL || mentioned || mentionedByTag || mentionedByPhone;
        ...
        if (!shouldForward) {
          logger.info(
            ...
            'Mensagem ignorada (sem @mention nativa e FORWARD_ALL=false)'
          );
          continue;
        }

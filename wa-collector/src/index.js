...
// Se não tiver WEBHOOK_URL no .env, usa o endpoint que você passou
const WEBHOOK_URL =
...
async function sendWebhook(payload) {
  if (!WEBHOOK_URL) {
    logger.warn('WEBHOOK_URL não definido — não será enviado nada ao backend');
    return;
  }
...
async function createSock() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    // Só queremos novas notificações, não histórico
    if (type !== 'notify') {
...
<body>
  <div class="card">
    ...
    <div class="subtitle">Conecte o número do WhatsApp para capturar mensagens de grupos.</div>

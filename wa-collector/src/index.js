...
// URL fixa (ou via .env se quiser sobrescrever)
const WEBHOOK_URL =
...
// Se quiser, pode habilitar FORWARD_ALL=true no .env para debug (mandar tudo)
...
if (!WEBHOOK_URL) {
  logger.warn('WEBHOOK_URL não definido — nada será enviado ao backend!');
}
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        if (msg.key.fromMe) {
          logger.debug({ jid }, 'Ignorando mensagem enviada pela própria instância');
          continue;
        }
        ...
        // Detecta se a instância foi mencionada
        let mentioned = false;
        if (Array.isArray(mentionedJids) && mentionedJids.length > 0) {
          for (const mj of mentionedJids) {
            const norm = jidNormalizedUser(mj);
            if (myJidForCompare && norm === myJidForCompare) {
              mentioned = true;
              break;
            }
          }
        ...
        // Só envia se foi mencionado (ou se FORWARD_ALL=true pra debug)
        const shouldForward = mentioned || FORWARD_ALL;
        ...
        logger.info(
          ...
          'Enviando webhook de mensagem (com mention)'
        );

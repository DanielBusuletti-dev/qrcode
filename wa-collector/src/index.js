...
// ----------- AQUI: função que inicia / reinicia o WhatsApp ------------
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection) {
      ...
      if (connection === 'close') {
        ...
        if (shouldReconnect) {
          // Em caso de 515 (restartRequired) ou outros, tenta reconectar
          logger.info('Will try to restart WhatsApp connection...');
          setTimeout(() => {
            startWhatsApp().catch((err) =>
              logger.error({ err }, 'Error restarting WhatsApp socket'),
            );
          }, 3000);
        } else {
          // loggedOut definitivo: precisa resetar sessão
          logger.error(
            'Logged out from WhatsApp (DisconnectReason.loggedOut). Use /instance/reset para escanear um novo QR.',
          );
        }
      }
    }
...
// inicia o WhatsApp pela primeira vez
startWhatsApp().catch((err) => {
  logger.error({ err }, 'Fatal error in startWhatsApp');
  process.exit(1);
});

// ------------------ HTTP server (painel + API) ------------------------

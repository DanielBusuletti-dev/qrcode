  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    for (const msg of messages) {
      try {
        ...
        // novo: nome de exibição da instância (para @NomeDoBot)
        const myDisplayName =
          sock?.user?.name ||
          sock?.user?.pushname ||
          null;
        ...
        // novo: menção pelo nome (@NomeDoBot ou só NomeDoBot)
        const mentionedByName = myDisplayName
          ? text.toLowerCase().includes(`@${myDisplayName.toLowerCase()}`) ||
            text.toLowerCase().includes(myDisplayName.toLowerCase())
          : false;
        ...
        // AGORA: qualquer uma dessas condições faz a mensagem ser encaminhada
        const matched = mentioned || tagMatched || numberInText;

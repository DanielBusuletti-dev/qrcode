...
// pega contextInfo de qualquer tipo conhecido
function getContextInfoFrom(msg) {
  if (!msg) return null;

  if (msg.extendedTextMessage?.contextInfo) {
    return msg.extendedTextMessage.contextInfo;
  }
  if (msg.imageMessage?.contextInfo) {
    return msg.imageMessage.contextInfo;
  }
  if (msg.videoMessage?.contextInfo) {
    return msg.videoMessage.contextInfo;
  }
  if (msg.documentMessage?.contextInfo) {
    return msg.documentMessage.contextInfo;
  }

  // algumas combinações mais aninhadas
  if (msg.ephemeralMessage?.message) {
    const inner = getContextInfoFrom(msg.ephemeralMessage.message);
    if (inner) return inner;
  }

  if (msg.viewOnceMessage?.message) {
    const inner = getContextInfoFrom(msg.viewOnceMessage.message);
    if (inner) return inner;
  }
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        const unwrapped = unwrapMessageContent(rawMessage);
        const text = extractText(unwrapped) || '';

        // 🔎 NOVO: tenta pegar contextInfo de qualquer lugar (inner OU outer)
        const contextInfo =
        ...
        // 2) Heurística antiga de procurar o número no texto (APENAS PARA LOG)

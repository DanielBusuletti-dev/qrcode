...
// ⚠️ NÃO usamos mais FORWARD_ALL para decidir envio.
// Tudo será enviado APENAS se a instância for mencionada.
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }: any) => {
    ...
    for (const msg of messages) {
      try {
        ...
        // ✅ Regra final: SÓ registra/enviar se foi realmente mencionado
        if (!mentioned) {
          logger.debug(
            { messageId, jid, fromDigits, mentioned },
            'Mensagem ignorada (instância não foi mencionada)'
          );
          continue;
        }

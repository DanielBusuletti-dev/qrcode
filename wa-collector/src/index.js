...
// Se quiser forçar mandar tudo pra debug: FORWARD_ALL=true no .env
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        // 1) Compara jid normalizado
        // 2) Compara só os dígitos (caso a menção seja ao contato com o número)
        let mentioned = false;
        ...
        // Regra final:
        // - se FORWARD_ALL=true -> manda tudo (útil pra debug)
        // - senão -> só se foi realmente mencionado
        const shouldForward = FORWARD_ALL || mentioned;

        if (!shouldForward) {
          logger.debug(
            { messageId, jid, fromDigits, mentioned, textPreview: text.slice(0, 80) },

  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        // 1) Mention oficial do WhatsApp via mentionedJid
        ...
        if (Array.isArray(mentionedJids) && mentionedJids.length > 0) {
          for (const mj of mentionedJids) {
            ...
            const normDigits = digits(norm);
            if (
              (myJidForCompare && norm === myJidForCompare) ||
              (myDigits && normDigits && normDigits === myDigits)
            ) {
        ...
        // 2) Heurística: se o texto tiver explicitamente o número da instância
        let mentionedByText = false;
        if (!mentionedByContext && myDigits && text) {
          const onlyDigits = text.replace(/\D/g, '');
          if (onlyDigits.includes(myDigits)) {

...
// REMOVIDO: import qrcode from 'qrcode-terminal';
...
const main = async () => {
  ...
  sock = makeWASocket({
    version,
    printQRInTerminal: false, // mantém false pra não logar QR no terminal
    auth: state,
    logger,
  ...
  sock.ev.on('connection.update', ({ qr, connection, lastDisconnect }) => {
    if (qr) {
      // 👉 só guarda o QR em memória pra servir via HTTP
      latestQR = qr;
      // REMOVIDO: qrcode.generate(qr, { small: true });
    }
    if (connection) {
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    for (const msg of messages) {
      try {
        ...
        // Se a mensagem menciona a própria instância por LID,
        // substitui o marcador @LID no texto pelo número real ou nome.
        if (mentioned) {
          const myNumberText = myDigits || digits(sock?.user?.id || '');
          const myDisplay =
            myNumberText || sock?.user?.name || sock?.user?.pushname || '';
          ...
          if (myDisplay) {
            ...
            text = text.replace(/@([0-9:@a-zA-Z]+)/g, (full, inner) => {
              ...
              if (
                ...
                mentionTokens.has(baseLid(cleaned))
              ) {
                return `@${myDisplay}`;
              }
              return full;
  ...
  <script>
    ...
    async function render(){
      ...
      try{
        const s=await api.status();
        elStatus.textContent=s.status;
        // Try to fetch QR first; if ok, fill and exit
        try{
          const { qr } = await api.qr();
          copy.value = qr;
          const resp = await fetch('/instance/qr.png?ts=' + Date.now());
          if (!resp.ok) throw new Error('no_qr');
          const blob = await resp.blob();
          img.src = URL.createObjectURL(blob);
          img.style.display = 'block';
          elHasQR.textContent = 'sim';
          return;
        }catch(_){/* continue below */}
        elHasQR.textContent=s.hasQR?'sim':'não';
        if(s.hasQR){
          try{
            const {qr}=await api.qr();
        ...
        try {
          const { qr } = await api.qr();
          copy.value = qr;
          // img.src set via PNG fetch above
          // show handled when PNG loads
          elHasQR.textContent = 'sim';
        } catch (_) {}
      }catch(e){
        elStatus.textContent='erro';

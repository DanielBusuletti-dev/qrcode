  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        const selfDigits =
          myDigits || normalizeJidToDigits(selfBareJid || '');
        ...
        if (Array.isArray(mentionedJids) && mentionedJids.length > 0) {
          for (const mj of mentionedJids) {
            ...
            const sameUser =
              areJidsSameUser(mj, selfBareJid || '') ||
              areJidsSameUser(mjNorm, selfBareJid || '') ||
              areJidsSameUser(mj, sock?.user?.id || '');
            const sameDigits =
              selfDigits && mjDigits && mjDigits === selfDigits;

            logger.info({ mj, mjNorm, mjDigits, selfBareJid, selfDigits, sameUser, sameDigits },
              'Comparando mentionedJid com meu numero'
            );

            if (sameUser || sameDigits) {
        ...
        // Fallback: se digitarem o número na mensagem (ex.: @5511...)
        const textDigits = digits(text);
        const mentionedByPhone =
          !!(selfDigits && textDigits && textDigits.includes(selfDigits));

        const mentioned = mentionedByContext || mentionedByPhone;

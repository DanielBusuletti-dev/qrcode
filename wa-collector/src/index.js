  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection) {
      ...
      if (connection === 'close') {
        ...
        if (loggedOut) {
          logger.error('Sessão foi deslogada, é preciso apagar auth/ e escanear novamente.');
          return;
        }
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    for (const msg of messages) {
      try {
        ...
        // Quem está logado
        const myJidForCompare = myJid || (sock?.user?.id ? jidNormalizedUser(sock.user.id) : null);
        ...
        if (!shouldForward) {
          // Log opcional (debug)
          // logger.debug({ messageId, jid, text }, 'Ignorando mensagem (não menciona e FORWARD_ALL=false)');
          continue;
        }
  ...
  try {
    ...
    if (req.url?.startsWith('/instance/qr') && req.method === 'GET') {
      ...
      if (req.url.startsWith('/instance/qr.png')) {
        ...
        const stream = QRCode.toFileStream(res, latestQR, {
          type: 'png',
          margin: 1,
          width: 260,
        });

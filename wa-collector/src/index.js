...
async function handleMessagesUpsert(upsert) {
  ...
  for (const msg of messages) {
    try {
      ...
      // Só consideramos menção REAL (via @ no WhatsApp)
...
async function createSock() {
  ...
  sock = makeWASocket({
    ...
    auth: state,
    logger,
    // VOLTEI A IMPRIMIR O QR NO TERMINAL / LOG
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      if (sock?.user?.id) {
        ...
        lastQrString = null; // sessão aberta, não precisa mais do QR atual
        logger.info(
